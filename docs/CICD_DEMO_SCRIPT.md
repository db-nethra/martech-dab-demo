...
2. Browser tabs, in order:
   ...
   - [Prod pipeline](https://fe-sandbox-classic-sandbox-h6k0yr.cloud.databricks.com/pipelines/674885e1-1ebb-463b-99a7-7f75081e9869?o=7474658873151457)
...
## 5:00–8:00 — Dev inner loop: creating a pipeline, fast and disposable

Frame it as creation, not just deployment. Say:

> When a developer builds a new pipeline, there's no separate packaging step.
> They branch this repo, add three things side by side — transformation code
> with its data-quality expectations, the pipeline and job declarations
> (including the trigger) in `resources/`, and tests plus mock-data
> generation — and the project is already deployable.

Then show the loop in the terminal:
...
developer's own synthetic data generated by the seed job — they iterate
edit → deploy → run on mock data in seconds. Say:
